...
import type { Questionnaire, SymptomEntry } from "@shared/types/questionnaire";
...
export default function ReviewScreen() {
  const { authHeader, signOut } = useAuth();
  ...
  const loadQueue = useCallback(async () => {
    try {
      const list = await api.get<Questionnaire[]>("/questionnaire", authHeader);
      setQueue(list);
      setQueueError(null);
    } catch {
  ...
  const loadQuestionnaire = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const data = await api.get<Questionnaire>(`/questionnaire/${questionnaireId}`, authHeader);
  ...
  async function handleSave() {
    ...
    try {
      const updated = await api.patch<Questionnaire>(
  ...
  async function handleConfirm() {
    ...
    try {
      const result = await api.post<ConfirmResult>(
        `/questionnaire/${questionnaire.id}/confirm`,
        undefined,
        authHeader
      );
  ...
  async function handleOpenPdf() {
    if (!pdfUrl || !authHeader) return;
    try {
      const res = await fetch(`${API_BASE_URL}${pdfUrl}`, { headers: authHeader });
      if (!res.ok) {
        setActionMessage({ text: describeError(new ApiError(res.statusText, res.status)), isError: true });
        return;
      }
      ...
      setActionMessage({ text: "PDFの取得に失敗しました。もう一度お試しください。", isError: true });
    }
  }
      ...
      <main className="rv-main">
        <header className="rv-header">
          ...
          <div className="rv-header-actions">
            ...
            <button type="button" className="rv-hbtn rv-hbtn-logout" onClick={() => void handleLogout()}>
              ログアウト
            </button>
          </div>
        </header>
        ...
        <footer className="rv-footer">
          ...
          <div className="rv-footer-actions">
            ...
            {!isConfirmed ? (
              ...
              pdfUrl && (
                <Button variant="primary" className="rv-btn-primary" onClick={() => void handleOpenPdf()}>
                  問診票PDFを印刷/ダウンロードする
                </Button>
              )
            )}
