...
| 翻訳・症状分類・問診生成 | Claude API＋翻訳API | 症状カテゴリ分類、問診質問の動的生成、多言語翻訳 |
| バックエンド | Node.js（Express）または Python（FastAPI） | 音声〜翻訳〜問診生成パイプラインの制御、API集約 |
| データベース／模擬病院システム | Firebase または Supabase | 問診データの保存、模擬電子カルテAPI（FHIR風JSON構造）への登録デモ |
| ホスティング | Vercel／Render等 | デモ環境の公開 |
